...
# The definition of Graph Neural Network
# train network
...
# === load dataset
dataset = MyInMemoryDataset(root='./dataset_edge01')
...
class SAGCN(nn.Module):
    def __init__(self, sag_rate_3):
        ...
        self.conv1 = GCNConv(200, 100)
        self.pool1 = SAGPooling(100, 0.5)
        self.conv2 = GCNConv(100, 100)
        self.pool2 = SAGPooling(100, 0.5)
        self.conv3 = GCNConv(100, 100)
        self.pool3 = SAGPooling(100, sag_rate_3)
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(num_select_sites // 2, 12))
...
def train_sagcn(sag_rate_3, LR):
    ...
    results_df.to_csv(output_dir + '/probas_sagcn_{}sites.csv'.format(str(num_select_sites)))
    perms_df = pd.DataFrame(perms)
    perms_df.to_csv(output_dir + '/perms_{}sites.csv'.format(str(num_select_sites)))
