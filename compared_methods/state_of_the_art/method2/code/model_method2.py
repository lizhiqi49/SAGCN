...
class Model_2(nn.Module):
    ...
    def forward(self, x_sample, x_graph):
        ...
        x_sample = self.linear2(x_sample)

        # interaction net part
        batch_size, num_node, _ = x_graph.shape
        ...
        batch = np.arange(batch_size).repeat(num_node)
        batch = torch.tensor(batch, dtype=torch.long).to(device)
        x_graph = x_graph.view(-1, node_feature_len)
        pooling_out = self.top_k_pooling(x_graph, edge_index, batch=batch)
        x_graph = torch.mean(pooling_out[0],dim=-1)
        x_graph = x_graph.view(batch_size, -1)

        # concatenate
        x = torch.cat((x_sample, x_graph), dim=1).view(batch_size, 1, -1)
        x = self.conv1(x)
        x = self.conv2(x)
