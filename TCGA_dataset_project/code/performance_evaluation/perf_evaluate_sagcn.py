...
def performance_evaluation_sagcn():
    ...
    for site in num_sites:
        base_dir = 'G:/SJTU/cancerMethy/new_edit/sagcn/results/'
        output_dir = 'G:/SJTU/cancerMethy/new_edit/sagcn/performance_evaluation/'
        proba_file = "probas_sagcn_{}sites.csv".format(str(site))
        proba_df = pd.read_csv(base_dir+proba_file, index_col=0)
        probas_nocalibrated = np.exp(proba_df.values[:,:12])
